function [data, N, batch, varargout] = new_t4bc__nii2radiomics__alg(path, ...
    ...
    if singlesubject == 0
        ...
        for n = 1:size(directories,2)
            ...
            [vol, mask, ~] = fetch__niftipaths(directories{n},...
                img__string, img__type);
...
%             header = load_nii_hdr(vol);
            ...
            volObjInit.data = squeeze(volume.img);
            roiObjInit.data = squeeze(maschera.img);
            ...
            if numel(size(volObjInit.data)) == 2
                temp = cat(3, volObjInit.data, volObjInit.data);
                volObjInit.data = temp;
                %volObjInit.data = single(reshape(volObjInit.data, [size(volObjInit.data) 1]));

                temp = cat(3, roiObjInit.data, roiObjInit.data);
                roiObjInit.data = temp;
                %roiObjInit.data = single(reshape(roiObjInit.data, [size(roiObjInit.data) 1]));
            end
            volObjInit.spatialRef = imref3d(size(volObjInit.data),0.5,0.5,0.5);
            roiObjInit.spatialRef = imref3d(size(volObjInit.data),0.5,0.5,0.5);
            % settings
            imParamScan = Init_param_per_scan;
            ...
            morpho_struct=morph2Dfeatures(vol,mask,path_exe,img__type);
            radiomics = computeRadiomics(volObjInit,roiObjInit,imParamScan,boxString);
            [radiomics_out,fields] = structure2array(radiomics);
            racat__features = structure_radiomics(morpho_struct,path_exe,radiomics_out,fields,img__type);
...
%                 img__type, patient__root);
    ...
    elseif singlesubject == 1
        ...
        try
            ...
            volObjInit.spatialRef = imref3d(size(volObjInit.data),0.5,0.5,0.5);
            roiObjInit.spatialRef = imref3d(size(volObjInit.data),0.5,0.5,0.5);
%             volObjInit.spatialRef = imref3d(size(volObjInit.data),1,1,0.5);
%             roiObjInit.spatialRef = imref3d(size(volObjInit.data),1,1,0.5);
            % settings
            imParamScan = Init_param_per_scan;
