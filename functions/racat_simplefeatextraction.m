function racat_feat_str = racat_simplefeatextraction(in__file, in__mask,...
    ...
    if strcmp(img__type, 'PET')
        in_config = strcat(path_exe,'\main_code\functions\RaCat\config__IBSI-C__PET.ini');
        in_paz = strcat(path_exe,'\main_code\functions\RaCat\patientInfo.ini');
        command_racat = strcat('"',path_exe,'"', '\main_code\functions\RaCat\RaCaT_v1.4.exe --ini ', ...
            {' '}, '"',in_config,'"', ' --img ', {' '},'"', in__file,'"', ' --voi ', ...
            {' '}, '"',in__mask,'"', ' --out ', {' '},'"', output_name,'"', ' --pat ', ...
            {' '}, '"',in_paz,'"');
    elseif strcmp(img__type, 'CT') || strcmp(img__type, 'TC')
        in_config = strcat(path_exe,'\main_code\functions\RaCat\config__IBSI-C__CT.ini');
        command_racat = strcat('"',path_exe,'"', '\main_code\functions\RaCat\RaCaT_v1.4.exe --ini ', ...
            {' '},'"', in_config,'"', ' --img ', {' '}, '"', in__file, '"', ' --voi ', ...
            {' '}, '"', in__mask, '"', ' --out ', {' '}, '"', output_name, '"');
    elseif strcmp(img__type, 'MAMMOGRAPHY') || strcmp(img__type, 'RX')
        in_config = strcat(path_exe,'\main_code\functions\RaCat\config__IBSI-B__2D__CT.ini');
        command_racat = strcat('"',path_exe,'"', '\main_code\functions\RaCat\RaCaT_v1.4.exe --ini ', ...
            {' '},'"', in_config,'"', ' --img ', {' '},'"', in__file,'"', ' --voi ', ...
            {' '},'"', in__mask,'"', ' --out ', {' '},'"', output_name,'"');
    elseif strcmp(img__type, 'CYBERKNIFE')
        in_config = strcat(path_exe, '\main_code\functions\RaCat\config__IBSI-C__MRI.ini');
        command_racat = strcat('"',path_exe,'"', '\main_code\functions\RaCat\RaCaT_v1.4.exe --ini ', ...
            {' '},'"', in_config,'"', ' --img ', {' '},'"', in__file,'"', ' --voi ', ...
            {' '},'"', in__mask,'"', ' --out ', {' '},'"', output_name,'"');
    elseif strcmp(img__type, 'ADC')
        in_config = strcat(path_exe, '\main_code\functions\RaCat\config__IBSI-C__ADC.ini');
        command_racat = strcat('"',path_exe,'"', '\main_code\functions\RaCat\RaCaT_v1.4.exe --ini ', ...
            {' '},'"', in_config,'"', ' --img ', {' '},'"', in__file,'"', ' --voi ', ...
            {' '},'"', in__mask,'"', ' --out ', {' '},'"', output_name,'"');
    elseif strcmp(img__type, 'T2')
        in_config = strcat(path_exe, '\main_code\functions\RaCat\config__IBSI-C__T2.ini');
        command_racat = strcat('"',path_exe,'"', '\main_code\functions\RaCat\RaCaT_v1.4.exe --ini ', ...
            {' '},'"', in_config,'"', ' --img ', {' '},'"', in__file,'"', ' --voi ', ...
            {' '},'"', in__mask,'"', ' --out ', {' '},'"', output_name,'"');
    elseif strcmp(img__type, 'US') || strcmp(img__type, 'RETINOGRAPHY')
        in_config = strcat(path_exe, '\main_code\functions\RaCat\config__IBSI-B__2D__MRI.ini');
        command_racat = strcat('"',path_exe,'"', '\main_code\functions\RaCat\RaCaT_v1.4.exe --ini ', ...
            {' '},'"', in_config,'"', ' --img ', {' '},'"', in__file,'"', ' --voi ', ...
            {' '},'"', in__mask,'"', ' --out ', {' '},'"', output_name,'"');
    end
    ...
    if strcmp(img__type, 'MAMMOGRAPHY') || strcmp(img__type, 'RX') || strcmp(img__type, 'RETINOGRAPHY') || strcmp(img__type, 'US') 
        morpho_struct=morph2Dfeatures(in__file,in__mask,path_exe);
    ...
    %                 elseif strcmp(family,'gldzmFeatures2D')
        ...
        racat_table = readtable(strcat(path_exe,'\main_code\functions\RaCat\dataframe_IBSI__RaCaT__nomenclature.csv'));
...
%         racat_feat_str=[morpho_struct; racat_feat_str];
    elseif strcmpi(img__type,'US')
        ...
        racat_feat_str=[morpho_struct; racat_feat_str];
    elseif strcmpi(img__type,'CYBERKNIFE')
        for i=1:tot_table1
            if strcmpi(racat_feat_str(i,6),'FB')
                racat_feat_str{i,7}='FBN:64';
            else
