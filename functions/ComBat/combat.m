...
function [bayesdata, stand_mean_output, var_pooled, gamma_star,...
    delta_star, parametric] = combat(dat, batch, mod, parametric)

    %
    % dat -> columns = subjects
    %        rows = features
    %
    ...
    stand_mean_output = stand_mean(:,1);
	...
	for i=1:n_batch
        ...
        if size(indices,2) > 1
            ...
            error('Error. There must be at least two samples for each batch.')
        end
	end
	...
	if parametric
        ...
        for i=1:n_batch
            indices = batches{i};
            temp = itSol(s_data(:,indices), gamma_hat(i,:),...
                delta_hat(i,:), gamma_bar(i), t2(i), a_prior(i),...
                b_prior(i), 0.001);
    ...
    if (1-parametric)
        ...
        for i=1:n_batch
            indices = batches{i};
            temp = inteprior(s_data(:,indices), gamma_hat(i,:),...
                delta_hat(i,:));
            gamma_star = [gamma_star; temp(1,:)];
            delta_star = [delta_star; temp(2,:)];
        end
	...
	bayesdata = s_data;
	j = 1;
	for i=1:n_batch % (i.e. for each "machine" or "site")
		indices = batches{i}; % Indices of the i-th batch
		bayesdata(:,indices) = (bayesdata(:,indices) -...
            (batch_design(indices,:)*gamma_star)') ./...
            (sqrt(delta_star(j,:))'*repmat(1,1,n_batches(i)));
		j = j+1;
        % batch_design is the same as batchmod
        % All information about variables of interest (mod) or machine/site
        % (batch) depends on the variables "i" and "j"
        % No further single-subject information about variables of interest
        % (mod) is needed
	end
	bayesdata = (bayesdata .* (sqrt(var_pooled) * repmat(1,1,n_array)))...
        + stand_mean;
    % No further single-subject information about variables of interest
    % (mod) is needed    
